		...
		foreach ( $attachments as $id => $attachment ) {
			...
			if ( 'on' === $show_lightbox ) {
				$image_output = sprintf(
					'<a class="%4$s" href="%1$s" data-mfp-src="%6$s"><img src="%1$s" data-src="%1$s" width="%7$s" height="%8$s" class="swiper-lazy" title="%2$s"%5$s data-dsm-image-description="%3$s" /><div class="swiper-lazy-preloader"></div></a>',
					esc_url( $attachment->image_src_full[0] ),
					esc_attr( $attachment->post_title ),
					esc_attr( $attachment->post_excerpt ),
					'on' === $show_lightbox ? 'dsm_image_carousel_lightbox' : '',
					'' !== $dsm_image_alt_text ? esc_attr( " alt={$dsm_image_alt_text}" ) : '',
					esc_url( $attachment->lightbox_image_src_full[0] ),
					esc_attr( $attachment->image_src_full[1] ),
					esc_attr( $attachment->image_src_full[2] )
				);
			} elseif ( '' !== $dsm_upload_gallery_custom_link_url ) {
				$image_output = sprintf(
					'<a href="%3$s" class="dsm_image_carousel_link" target="%4$s" %5$s><img src="%1$s" data-src="%1$s" width="%7$s" height="%8$s" class="swiper-lazy" title="%2$s"%6$s /><div class="swiper-lazy-preloader"></div></a>',
					esc_url( $attachment->image_src_full[0] ),
					esc_attr( $attachment->post_title ),
				...
				$image_output = sprintf(
					'<img src="%1$s" data-src="%1$s" width="%4$s" height="%5$s" class="swiper-lazy" title="%2$s"%3$s /><div class="swiper-lazy-preloader"></div>',
					esc_url( $attachment->image_src_full[0] ),
					esc_attr( $attachment->post_title ),
					'' !== $dsm_image_alt_text ? esc_attr( " alt={$dsm_image_alt_text}" ) : '',
					esc_attr( $attachment->image_src_full[1] ),
